...
(function () {
    ...
    class LoginController {
        ...
        showMicrosoftLogin(event) {
            ...
            var x = screen.width/2 - 790/2;
            var y = screen.height/2 - 550/2;
            // open new login window
            this._loginWindow = window.open(
                this._loginURL,
                'microsoft-login',
                'height=560, width=790, left=' + x +', top=' + y + ', menubar=no, location=no, resizable=no, scrollbars=yes, status=no, toolbar=no'
            );
        }
            ...
            if (event.origin === origin) {
                if (event.data.microsoft_auth) {
                    ...
                    if (data.error) {
                        ...
                        var loginContainer = document.getElementById('login-error-alert');
